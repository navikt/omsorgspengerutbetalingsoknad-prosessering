...
data class Cleanup(val metadata: Metadata, val preprosessertMelding: PreprossesertMeldingV1, val journalførtMelding: Journalfort)
data class Journalfort(val journalpostId: String, val søknad: Søknad)

internal object Topics {
    val MOTTATT_V2 = Topic(
        name = "dusseldorf.privat-omsorgspengerutbetalingsoknad-mottatt-v2",
        serDes = SerDes()
    )
    ...
    val CLEANUP = Topic(
        name = "dusseldorf.privat-omsorgspengerutbetalingsoknad-cleanup",
        serDes = SerDes()
    )
...
internal fun TopicEntry.deserialiserTilCleanup(): Cleanup  = omsorgspengerutbetalingsoknadKonfigurertMapper().readValue(data.rawJson)
internal fun TopicEntry.deserialiserTilMelding(): MeldingV1 = omsorgspengerutbetalingsoknadKonfigurertMapper().readValue(data.rawJson)
internal fun TopicEntry.deserialiserTilPreprosessertMelding(): PreprossesertMeldingV1 = omsorgspengerutbetalingsoknadKonfigurertMapper().readValue(data.rawJson)
internal fun Any.serialiserTilData() = Data(omsorgspengerutbetalingsoknadKonfigurertMapper().writeValueAsString(this))
...
fun omsorgspengerutbetalingsoknadKonfigurertMapper(): ObjectMapper {
